...
import { fetchGemini, fetchGeminiStream } from './providers/gemini';
import { fetchGrok, fetchGrokStream } from './providers/grok';
import { fetchOpenAI, fetchOpenAIStream } from './providers/openai';
 ...
 * Calls onChunk with each text delta as it arrives from the provider
 */
export async function callAIStream(
    ...
    if (isGrok) {
        return await fetchGrokStream(grokKey, model, systemPrompt, userPrompt, onChunk);
    } else if (isOpenAI) {
        return await fetchOpenAIStream(openaiKey, model, systemPrompt, userPrompt, onChunk);
    } else {
        return await fetchGeminiStream(gemKey, model || 'models/gemini-2.5-flash', systemPrompt, userPrompt, onChunk);
    }
}
